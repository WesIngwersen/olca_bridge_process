# Script to create a bridge processes in an openlca database. The contents of
# this file could be pasted into the Python interpreter in the openLCA database.
# The processes and associated reference flows to which the bridge processes
# link should already be present in the openlca database before running
# the script.
...
# Function to store the input flow found by the SQL query. If several
# matches are found, only the last one will be stored
def callbackInput(rs):
    processFlows['Input'] = rs.getInt(1)


# Function to store the output flow found by the SQL query. If several
# matches are found, only the last one will be stored
...
# Modify it depending on the defined criteria (e.g. name, location,
# category, provider, etc.)
def find_flowInput(name):
    olca.querySql(
        "SELECT tbl_flows.id FROM tbl_flows WHERE tbl_flows.name = '" +
        name + "'", callbackInput)


# Function to get the output flow. If several criteria apart from the name
# need to be fulfilled (e.g. location) a SQL query is run.
# Modify it depending on the defined criteria (e.g. name, location,
# category, provider, etc.)
...
# Function to create a new actor if any match for the name in the csv is
...
with open(csvFile, 'rb') as f:
    ...
    for row in data:
        log.info("create process {}", row['BridgeProcessName'])
        process = Process()
        ...
        # If more data were available in the csv file (e.g. category, time,
        # geography, etc.), the correspondent process documentation fields
        # should be completed
        olca.insertProcess(process)
        # If any flow is not found, an error is logged and its exchange will
        # not be created
        find_flowInput(row['InputName'])
        if 'Input' not in processFlows:
            log.error('Flow: "' + row['InputName'] +
                      ' does not exist in database')
        find_flowOutput(row['OutputName'])
        if 'Output' not in processFlows:
            log.error('Flow: "' + row['OutputName'] +
                      ' does not exist in database')
        for key, value in processFlows.items():
            ...
            if key == 'Input':
                ...
                exchange.amount = float(row['InputAmount'])
                unitCol = 'InputUnit'
            elif key == 'Output':
                exchange.isInput = False
                exchange.amount = float(row['OutputAmount'])
                unitCol = 'OutputUnit'
            for factor in exchange.flow.flowPropertyFactors:
                for unit in factor.flowProperty.unitGroup.units:
                    if unit.name == row[unitCol]:
                        exchange.unit = unit
                        exchange.flowPropertyFactor = factor
            # If the unit specified do not match with any flow property factor
            # of the flow, the exchange is not created
            if exchange.unit is None:
                log.error('Unit: "' + row[unitCol] +
                          '" cannot be used by flow: "' +
                          exchange.flow.name + '" - ' +
                          exchange.flow.location.code)
            else:
